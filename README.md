...
- **Selected-scene preview** — the Preview control now renders only the selected scene through the authoritative Canvas/video/whiteboard pipeline, includes persisted narration, honors scene trim/speed, and intentionally excludes project ordering/transitions.
- **Scene lifecycle timeline integrity** — Add, Duplicate, Move and Delete now preflight persisted timeline state, keep `mediaOrder` synchronized with authoritative scene order, prune transitions that cease to be adjacent, prune deleted-scene edits, clone the source `sceneEdits` window when duplicating, and fail closed if opaque legacy track buckets are populated.
...
This release includes the recovered generated-image workflow, capability-driven advanced voice workflow, persisted additive scene-transition editor/export path, per-scene narration in authoritative final export, direct editing of the recovered per-object pause/draw/push whiteboard timing contract, scene-level trim/start-end/speed editing through the rebuild-owned `sceneEdits` contract, deterministic readable-label materialization from AI Analyze grounded labels/boxes, selected-scene preview through the same authoritative render path, and fail-closed scene-lifecycle reconciliation for supported persisted timeline state. It does **not** yet claim parity for arbitrary custom draw paths, source-exact non-default hand/background-removal behavior, arbitrary legacy multi-track clip splitting/cutting or decoded non-empty `clips` / `videoClips` / `audioClips` entry schemas, object-SFX semantics, or source-exact legacy label typography/decoration beyond the recovered grounded-box placement authority. Those remain subsequent parity layers rather than being hidden behind a false “finished” claim.
