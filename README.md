...
This release includes the recovered generated-image workflow, capability-driven advanced voice workflow, persisted additive scene-transition editor/export path, per-scene narration in authoritative final export, and direct editing of the recovered per-object pause/draw/push whiteboard timing contract. It does **not** yet claim parity for arbitrary custom draw paths, source-exact non-default hand/background-removal behavior, unrecovered multi-track trimming/cutting, object-SFX semantics, or automatic exact-label post-processing whose source/layout contract has not yet been recovered strongly enough. Those remain subsequent parity layers rather than being hidden behind a false “finished” claim.
