...
@pytest.mark.parametrize(
    ...
    [
        "custom_draw_points",
        "custom_object_push_config",
        "custom_object_effect_config",
        "custom_object_sound_config",
        "batch_voice_segments",
    ],
)
def test_render_config_rejects_empty_mapping_for_list_render_fields(field):
    with pytest.raises(
        InvalidRenderConfig,
        match=rf"^{field} must be a list$",
    ):
        normalize_render_config({field: {}})
...
def test_project_exporter_rejects_later_empty_mapping_list_render_field_before_any_render(
    tmp_path,
    field,
):
    store = ProjectStore(tmp_path / f"{field}-shape.db")
    store.initialize()
    store.create_project("p1", "Structured list-field preflight")
    store.replace_scenes("p1", [Scene(0, "First"), Scene(1, "Second")])
    first, second = store.list_scenes("p1")
    ...
    output = tmp_path / f"never-{field}-shape.mp4"

    with pytest.raises(
        InvalidRenderConfig,
        match=rf"^scene {second['id']} {field} must be a list$",
    ):
        try:
            exporter.export("p1", output)
        finally:
...
@pytest.mark.parametrize(
    ("field", "value", "message"),
    [
        ("style", "legacy_canvas", "style must be whiteboard or color_reveal"),
        ("visual_mode", "orbit", "visual_mode must be drawing or camera_motion"),
    ],
)
def test_render_config_rejects_explicit_unsupported_primary_render_modes(
    field,
    value,
    message,
):
...
def test_project_exporter_rejects_later_unsupported_primary_render_mode_before_any_render(
    tmp_path,
    field,
    value,
    message,
):
    store = ProjectStore(tmp_path / f"{field}-unsupported.db")
    store.initialize()
    store.create_project("p1", "Primary render mode preflight")
    store.replace_scenes("p1", [Scene(0, "First"), Scene(1, "Second")])
    first, second = store.list_scenes("p1")
    ...
    output = tmp_path / f"never-{field}-unsupported.mp4"

    with pytest.raises(
        InvalidRenderConfig,
        match=rf"^scene {second['id']} {message}$",
    ):
        try:
            exporter.export("p1", output)
        finally:
