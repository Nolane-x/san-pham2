...
def _video_plan(**geometry: float) -> SceneRenderPlan:
    video = {
        "id": "video",
        "kind": "video",
        "source": "source.mp4",
...
def test_video_geometry_preflight_rejects_nonfinite_values(field, value):
    with pytest.raises(
        UnsupportedVideoComposition,
        match=rf"^video {field} must be finite$",
    ):
        validate_supported_video_composition(_video_plan(**{field: value}))
...
def test_video_geometry_preflight_accepts_finite_values(geometry):
    validate_supported_video_composition(_video_plan(**geometry))


def test_project_exporter_preflights_later_nonfinite_video_geometry_before_any_render(tmp_path):
    store = ProjectStore(tmp_path / "studio.db")
    store.initialize()
    store.create_project("p1", "Non-finite video geometry preflight")
    ...
    with pytest.raises(UnsupportedVideoComposition, match=r"^video width must be finite$"):
