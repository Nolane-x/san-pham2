...
@pytest.mark.parametrize("mode", ["custm", "automatic", "unknown"])
def test_object_timing_rejects_unsupported_mode(mode):
    with pytest.raises(
        InvalidObjectTiming,
        match=rf"^object_timing_mode must be fixed or custom$",
    ):
        build_render_timing_plan(
            [{"id": "object-a", "visible": True}],
            {
                "object_timing_mode": mode,
                "reveal_duration": 1.0,
            },
        )


@pytest.mark.parametrize("mode", ["fixed", "custom"])
def test_object_timing_accepts_supported_modes(mode):
    config = {
        "object_timing_mode": mode,
        "reveal_duration": 1.0,
    }
    if mode == "custom":
        config["custom_object_timing_config"] = [
...
def test_project_exporter_rejects_later_unsupported_object_timing_mode_before_any_render(
    tmp_path,
):
    store = ProjectStore(tmp_path / "unsupported-timing-mode.db")
    store.initialize()
    store.create_project("p1", "Unsupported object timing mode preflight")
    store.replace_scenes("p1", [Scene(0, "First"), Scene(1, "Second")])
    first, second = store.list_scenes("p1")
    ...
    with pytest.raises(
        InvalidObjectTiming,
        match=rf"^scene {second['id']} object_timing_mode must be fixed or custom$",
    ):
        try:
            exporter.export("p1", tmp_path / "never-unsupported-mode.mp4")
        finally:
            assert render_calls == []
            assert media.calls == []
...
def test_custom_object_timing_rejects_entry_without_object_identity(entry):
    with pytest.raises(
        InvalidObjectTiming,
        match=r"^custom timing entry 0 must identify an object$",
    ):
        build_render_timing_plan(
            [{"id": "object-a", "visible": True}],
            {
...
def test_custom_object_timing_keeps_stale_nonvisible_identity_nonblocking():
    plan = build_render_timing_plan(
        [{"id": "object-a", "visible": True}],
        {
            "object_timing_mode": "custom",
            "reveal_duration": 1.0,
            "custom_object_timing_config": [
                {"object_id": "stale-object", "draw": 0.25}
            ],
        },
    )
    assert len(plan) == 1
...
def test_project_exporter_rejects_later_identityless_custom_timing_before_any_render(
    tmp_path,
):
    store = ProjectStore(tmp_path / "identityless-timing.db")
    store.initialize()
    store.create_project("p1", "Identityless object timing preflight")
    store.replace_scenes("p1", [Scene(0, "First"), Scene(1, "Second")])
    first, second = store.list_scenes("p1")
    ...
    store.update_scene_render_settings(
        ...
        settings={
            "style": "whiteboard",
            "object_timing_mode": "custom",
            "custom_object_timing_config": [{"draw": 0.25}],
        },
    )
    ...
    with pytest.raises(
        InvalidObjectTiming,
        match=rf"^scene {second['id']} custom timing entry 0 must identify an object$",
    ):
        try:
            exporter.export("p1", tmp_path / "never-identityless.mp4")
        finally:
            assert render_calls == []
            assert media.calls == []
